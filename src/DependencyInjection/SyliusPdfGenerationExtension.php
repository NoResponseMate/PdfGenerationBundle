...
final class SyliusPdfGenerationExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        $container->setParameter('sylius_pdf_generation.gotenberg.base_url', $gotenbergBaseUrl);
        ...
        if ([] !== $deferredAdapterContexts) {
            $container->setParameter('.sylius_pdf_generation.deferred_adapter_contexts', $deferredAdapterContexts);
        }
        ...
        $container->getDefinition('sylius_pdf_generation.manager')
    ...
    private function registerAdapter(
        ...
        $adapterServiceId = sprintf('sylius_pdf_generation.adapter.%s', $contextName);

        $container->setDefinition(
            $adapterServiceId,
            (new ChildDefinition(sprintf('sylius_pdf_generation.adapter.%s', $adapterName)))
    ...
    private function registerStorage(
        ...
        $serviceId = sprintf('sylius_pdf_generation.storage.%s', $context);
    ...
    private function registerProcessor(
        ...
        $parentId = sprintf('sylius_pdf_generation.options_processor.%s', $adapterName);

        if (!$container->hasDefinition($parentId)) {
            return;
        }

        $processorServiceId = sprintf('sylius_pdf_generation.options_processor.%s.%s', $adapterName, $contextName);
        ...
        $processorDefinition->addTag('sylius_pdf_generation.options_processor', $tagAttributes);
    ...
    {
        $compositeId = sprintf('sylius_pdf_generation.options_processor.composite.%s', $adapterName);
