...
final class GaufretteStorageTest extends KernelTestCase
{
    ...
    private string $gaufretteDirectory;
    ...
    {
        $kernel = self::bootKernel(['environment' => 'test_gaufrette']);

        /** @var string $cacheDir */
        $cacheDir = $kernel->getContainer()->getParameter('kernel.cache_dir');
        $this->gaufretteDirectory = $cacheDir . '/gaufrette';
    }
    ...
    protected function tearDown(): void
    {
        ...
        $pdfDir = $this->gaufretteDirectory . '/pdf';
        if (is_dir($pdfDir)) {
            $files = glob($pdfDir . '/*');
            if (false !== $files) {
        ...
        $cacheDir = dirname($this->gaufretteDirectory) . '/pdf_local_cache';
        if (is_dir($cacheDir)) {
    ...
    public function it_saves_a_pdf_file(): void
    {
        ...
        self::assertFileExists($this->gaufretteDirectory . '/pdf/test-save.pdf');
    }
    ...
    public function it_removes_a_pdf_file(): void
    {
        ...
        self::assertFileDoesNotExist($this->gaufretteDirectory . '/pdf/to-remove.pdf');
    }
    ...
    public function it_stores_files_under_the_configured_prefix(): void
    {
        ...
        self::assertFileExists($this->gaufretteDirectory . '/pdf/prefixed.pdf');
    }
