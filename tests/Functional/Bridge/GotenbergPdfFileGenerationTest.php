...
final class GotenbergPdfFileGenerationTest extends KernelTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        if (!self::isGotenbergAvailable()) {
            self::markTestSkipped('Gotenberg server is not available at http://localhost:3000.');
        }
    }
    ...
    {
        $kernel = self::bootKernel(['environment' => 'test_gotenberg']);
    ...
    private static function isGotenbergAvailable(): bool
    {
    ...
    {
        $renderer = self::getContainer()->get('test.sylius_pdf.renderer.twig');
    ...
    {
        $renderer = self::getContainer()->get('test.sylius_pdf.renderer.html');
    ...
    {
        $generator = self::getContainer()->get('test.sylius_pdf.generator');
